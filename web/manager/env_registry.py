...
ENV_REGISTRY: tuple[EnvVarDef, ...] = (
    ...
    EnvVarDef(
        "TOURNAMENT_INTRO_SECONDS",
        "Battle & agents",
        "Seconds the tournament intro card stays up before the first match starts (same delay before run_single_match). 0 disables.",
        "0",
        "agents",
    ),
    EnvVarDef(
        "TOURNAMENT_INTRO_DELAY_SECONDS",
        "Battle & agents",
        "Seconds to delay after the tournament intro card hold (intro_gap phase) before starting the first match — use for fade-out + buffer before match intro. 0 skips.",
        "0",
        "agents",
    ),
    EnvVarDef(
        "MATCH_INTRO_STARTING_HOLD_SECONDS",
        "Battle & agents",
        "After writing battle status starting, wait this long before opening the Showdown battle so /broadcast can poll starting (default beats 250ms hub + connect). 0 skips.",
        "0.45",
        "agents",
    ),
    EnvVarDef(
    ...
    EnvVarDef(
        "LEARNINGS_UPDATE_INTERVAL",
        "Persona memory",
        "When reflection runs, update learnings.md every N matches per persona (0 = skip learnings updates).",
        "3",
        "agents",
    ),
    EnvVarDef(
        "MAX_MEMORY_ENTRIES",
        "Persona memory",
        "Maximum battle diary sections kept in memory.md.",
        "10",
        "agents",
    ),
    EnvVarDef(
        "MAX_LEARNINGS_BULLETS",
        "Persona memory",
        "Maximum bullet lines kept when trimming learnings.md.",
        "30",
        "agents",
    ),
    EnvVarDef(
        "LLM_MEMORY_REFLECTION_MAX_TOKENS",
        "Persona memory",
        "Max output tokens for post-match reflection JSON.",
        "2048",
        "agents",
    ),
    EnvVarDef(
    ...
    EnvVarDef(
        "TOURNAMENT_VICTORY_MODAL_SECONDS",
        "Web",
        "Victory splash duration when the match clinches the tournament (seconds).",
        "60",
        "web",
    ),
    EnvVarDef(
    ...
    EnvVarDef(
        "PORTRAITS_DIR",
        "Paths",
        "Persona portraits (tall + square/) for manager and broadcast; bind-mounted from assets.",
        "/app/static/portraits",
        "web",
    ),
    EnvVarDef(
        "REPLAY_DIR",
